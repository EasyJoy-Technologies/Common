 ...
 */
public class GroMoreSplashMinWindowManager {
    ...
    private volatile static GroMoreSplashMinWindowManager mInstance;
     ...
     * @return
     */
    public static GroMoreSplashMinWindowManager getInstance(Context context) {
        if (mInstance == null) {
            synchronized (GroMoreSplashMinWindowManager.class) {
                if (mInstance == null) {
                    mInstance = new GroMoreSplashMinWindowManager(context);
                }
            }
        }
        return mInstance;
    ...
    private GroMoreSplashMinWindowManager(Context context) {
        this.context = context;
        initMinWindowViewData(context);
        mMinWindowViewMargin = UIUtils.dp2px(context, 16);
        mMinWindowViewMarginBottom = UIUtils.dp2px(context, 100);
        mMinWindowViewPos = RIGHT;
        mMinWindowViewAnimationTime = 300;
    }

    private void initMinWindowViewData(Context context) {
        int deviceWidth = Math.min(UIUtils.getScreenWidthInPx(context), UIUtils.getScreenWidthInPx(context));
        if (mSplashAd != null && mSplashAd.getMinWindowSize() != null && mSplashAd.getMinWindowSize().length >= 2) {
            //使用推荐的点睛宽高
            mMinWindowViewWidth = UIUtils.dp2px(context, mSplashAd.getMinWindowSize()[0]);
            mMinWindowViewHeight = UIUtils.dp2px(context, mSplashAd.getMinWindowSize()[1]);
        } else {
            //默认的点睛宽高
        ...
        rect.bottom = UIUtils.getScreenHeightInPx(context) - mMinWindowViewMarginBottom;
    ...
    public ViewGroup showMinWindowInTwoActivity(final ViewGroup decorView,
         ...
         */
        if (mSplashAd.showWindowDirect(rect, mTTSplashAdListener)) {
            return null;
        }
    ...
    private ViewGroup startSplashClickEyeAnimationInTwoActivity(final ViewGroup decorView,
        ...
        return startSplashClickEyeAnimation(mSplashAd, mSplashShowView, decorView, splashViewContainer, callBack);
    }
    ...
    private ViewGroup startSplashClickEyeAnimation(final GMSplashAd splashAd, final View splash, final ViewGroup decorView,
        ...
        if (minWindowSize != null && minWindowSize.length >= 2) {
            //使用推荐的开屏小窗宽高
            mMinWindowViewWidth = UIUtils.dp2px(context, minWindowSize[0]);
            mMinWindowViewHeight = UIUtils.dp2px(context, minWindowSize[1]);
        }
     ...
     * 开屏广告监听，针对快手开屏小窗视图使用
     */
    TTSplashAdListener mTTSplashAdListener = new TTSplashAdListener() {
        @Override
